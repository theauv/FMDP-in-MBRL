...
for model in "linear_regression" "simple"
    ...
    for learned_reward in false true
        ...
        sbatch -n 1 --cpus-per-task=2 --time=24:00:00 --mem-per-cpu=1024 --output="output/%J" --wrap="python3 run_scripts/train_model.py overrides=pets_bikes debug_mode=false learned_rewards=${learned_reward} dynamics_model=${model} run_name=${run_name}"
    done
done
