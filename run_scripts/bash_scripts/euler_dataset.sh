...
for sparsity in null "src/env/bikes_data/acbo_centroids/factors_radius_5.npy" "src/env/bikes_data/acbo_centroids/factors_radius_3.npy"
    ...
    for n_actions in 1 2 4
        ...
        sbatch -n 1 --cpus-per-task=2 --time=24:00:00 --mem-per-cpu=1024 --output="output/%J" --wrap="python3 run_scripts/create_datasets.py overrides=pets_bikes debug_mode=false overrides.env_config.centroids_coord=${centroids}  overrides.env_config.station_dependencies=${sparsity}"
    done
done