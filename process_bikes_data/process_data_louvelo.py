...
trips_centroids_coords = (
    trips_centroids_coords.reset_index()
)  # make sure indexes pair with number of rows
num_centroids = (
    max(
        trips_centroids_coords["Start_Station_Id"].max(axis=0),
        trips_centroids_coords["End_Station_Id"].max(axis=0),
    )
    + 1
)
...
for index, row in trips_centroids_coords.iterrows():
    ...
    if not math.isnan(start_station_id):
        ...
        if (
            not math.isnan(start_latitude)
            and centroids_count[start_station_id, 0] == 0.0
        ):
            centroids_count[start_station_id, 0] = 1.0
            centroids_coords[start_station_id, 0] = start_latitude
        if (
            not math.isnan(start_longitude)
            and centroids_count[start_station_id, 1] == 0.0
        ):
            centroids_count[start_station_id, 1] = 1.0
    ...
    if not math.isnan(end_station_id):
        ...
        if not math.isnan(end_latitude) and centroids_count[end_station_id, 0] == 0.0:
            centroids_count[end_station_id, 0] = 1.0
            centroids_coords[end_station_id, 0] = end_latitude
        if not math.isnan(end_longitude) and centroids_count[end_station_id, 1] == 0.0:
            centroids_count[end_station_id, 1] = 1.0
            centroids_coords[end_station_id, 1] = end_longitude

    if np.all(centroids_count):
        break

centroids_coords = centroids_coords[~np.all(centroids_coords == 0.0, axis=1)]
...
all_trips_data = all_trips_data.drop(
    [
        "X",
        "Y",
        "Closed_Status",
...
all_trips_data = all_trips_data.rename(
    columns={
...
tqdm.pandas()
all_trips_data["DayOfWeek"] = all_trips_data.progress_apply(
    lambda x: ((x.name + 1) % 7) + 1, axis=1
)
all_trips_data["HourNum"] = all_trips_data.progress_apply(
    lambda x: int(x["StartTime"].split(":")[0]), axis=1
)
